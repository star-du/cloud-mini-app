...
QRCode.prototype = {
  ...
  mapData: function (data, maskPattern) {
    ...
    this.rawModule = Array(this.moduleCount);
    for (let i = 0; i < this.moduleCount; i++) this.rawModule[i] = Array(this.moduleCount);
...
(() => {
  let i;
  for (i = 0; i < 8; i++) QRMath.EXP_TABLE[i] = 1 << i;
  for (i = 8; i < 256; i++) {
...
QRRSBlock.getRsBlockTable = function (typeNumber, errorCorrectLevel) {
  switch (errorCorrectLevel) {
    case QRErrorCorrectLevel.L: return QRRSBlock.RS_BLOCK_TABLE[typeNumber * 4 - 4];
    case QRErrorCorrectLevel.M: return QRRSBlock.RS_BLOCK_TABLE[typeNumber * 4 - 3];
    case QRErrorCorrectLevel.Q: return QRRSBlock.RS_BLOCK_TABLE[typeNumber * 4 - 2];
    case QRErrorCorrectLevel.H: return QRRSBlock.RS_BLOCK_TABLE[typeNumber * 4 - 1];
...
var DorQRcode = function () {
  ...
  //画布初始化
  var canvasInit = function (dom) {
    // canvas = document.createElement('canvas');
    // dom.width = config.width;
    // dom.height = config.height;
    // ctx = canvas.getContext('2d');
    ctx = wx.createCanvasContext(dom);
  }

  // 二维码数据初始化 from QRCode
  ...
  var drawCode = function (type, opt, isShadow) {
    ...
    if (type == "round") {
      ...
      //单点设定颜色
      var isDark = qrcode.isDark(row, col);
      const d = isNaN(raw.rawModule[row][col]) ? (isDark ? "1" : "0") :
        (raw.rawModule[row][col] ? "1" : "0");
      if (isDark) {
        if (config.pointColor) {
        ...
        if (fillBit) {
          ...
          ctx.fillText(d, Math.round(col * tileW) + w / 2 - 5, Math.round(row * tileH) + h / 2 + 5);
          ctx.fillStyle = tmp;
        }
  ...
  var init = function (dom, options) {
    ...
    config.correctLevel = options.correctLevel || QRCode.QRErrorCorrectLevel.H;
    ...
    const getConfigColor = (name, opt, defaultVal) => {
      ...
      if (opt) {
        switch (typeof opt) {
          case "string":
            ...
            break;
          case "object":
            let linear = ctx.createLinearGradient(0, 0, 0, config.width);
            for (let em in opt) linear.addColorStop(Number(em), opt[em]);
            config[name] = linear;
            break;
        }
      }
    ...
    getConfigColor("background", options.background, "#fff"); // 背景色
    getConfigColor("color", options.color, "#000"); // 前景色
...
DorQRcode.QRErrorCorrectLevel = QRErrorCorrectLevel;

module.exports = DorQRcode;